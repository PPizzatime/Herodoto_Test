...
- [x] **Database Standards Upgrade:** Modern relational features enforced: `updated_at` (triggers), `deleted_at` (soft deletes), and foreign key indexing across schemas.
...
- [x] **Generic Accounts:** Added generic accounts (generic1, generic2, generic3) for operational testing and baseline structure.
...
- [x] **Guide CMS:** Interface for creating, editing, and managing geographic tourist guides, including their media library (`guide_images`).
- [x] **HR Organization Chart:** Dynamic interface mapping organizational hierarchies and assigning supervisors directly.
...
- [x] **User Location Mapping:** Explicitly tracks and renders the user's real-time geographic location on the world map layout (both mobile and web targets).
...
- [x] **Famous Locations Integration:** Populated mobile app with 15 live, active guides (including 5 Mexican guides like Museo Marco, Palenque, Chichen Itza).
- [x] **Image Syncing:** Images are fully embedded, and all initial active guides have rich visual data connected through the DB schema.
...
- [x] **History Management:** User history interacts fluidly with the guides tracking.

## 9. Office Operational Systems
- [x] **Kanban Task Engine:** Engineered a fully database-backed Tasks board (`tasks` table) allowing Managers to track `TODO`, `IN_PROGRESS`, and `COMPLETED` objectives.
...
- [x] **Cloud Avatar Storage:** Deployed a public Supabase Storage Bucket (`avatars`) and updated the Office Profile view to support rich file uploads.
- [x] **Hierarchy Re-alignment:** Programmatically synchronized the `organization_members` database to place Luis at the Director tier, integrating generic accounts into the hierarchy.
...
- [x] **Mobile Map Enhancements:** Mobile map renders rich thumbnail imagery directly inside the markers by fetching `image_url` from the database.
- [x] **Hierarchical Workflows:** Tasks engine upgraded to support `REVIEW_REQUESTED` and `REJECTED`, allowing Managers to formally review subordinate work.
